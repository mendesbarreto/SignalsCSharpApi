...
{
    public interface IPrioritySignal<T> : ISignal<T>
    {
        /**
		 ...
		 * @see ISlot
		 */
        ISlot<T> AddWithPriority(OnceSignal<T>.SignalDelegateArgTemplateCallback1 listener, int priority = 0);
		 ...
		 * @see ISlot
		 */
        ISlot<T> addOnceWithPriority(OnceSignal<T>.SignalDelegateArgTemplateCallback1 listener, int priority = 0);
    }
}
